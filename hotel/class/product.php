...
class product extends model
{
	...
	public function list_all_product($keyword, $shop_id, $member_group_id, $offset, $limit)
	{
		...
		$sql = "SELECT pro.`id` as product_id , pro.`name` as product_name, pro.`unit_import`, pro.`unit_export`, pro.`on_sales`, pro.`web_id`,
				...
				pro.`img_1`, pro.`img_2`, pro.`img_3`, pro.`img_4`, pro.`img_5`, pro.`img_6`, 
				pro.`sum_stock`, pro.`description`, pro.`short_description`, pro.`point`, pro.`for_point`, pro.`hidden`,
				SKU.`code`, SKU.`on_stock`, SKU.`id` as sku_id, pro.`price`, pro.`wholesale_price`,
				IFNULL(SKU.`name`, '') sku_name, IFNULL(SKU.`img_1`, '') sku_img_1, SKU.`unique_id`, pro.`product_commission_id`, SKU.`price_promotion`, SKU.`price` sku_price
				FROM $db->tbl_fix`product` as pro
				INNER JOIN `SKU` ON pro.`id` = SKU.`product_id` 
	...
	public function detail_product_general($product_commission_id, $member_group_id, $dProduct ){
		global $db, $setup, $main, $link, $dClient;
		...
		$dProduct['member_group_price'] 	= 0;
		$dProduct['decrement'] 				= 0;
		$dProduct['price_cal_commission']	= 0;//Giá để tính chiết khấu: Dựa vào giá lẻ giá sỉ hay discount
		if( isset($setup['enabled_apply_discount_on_price']) && $setup['enabled_apply_discount_on_price'] == 1 ){
			$product_price_detail->set('product_id', $dProduct['product_id']);
			...
			$dProduct['price_cal_commission'] = $dProduct['member_group_price'];
		}else{
			/**
			...
			$dProduct['price_cal_commission'] = $dProduct['price']*(100-$dProduct['decrement'])/100;
			/**
		...
		$dProduct['cashback_vnd'] 		= 0;//mới cập nhật
		$dProduct['cashback_point'] 	= 0;//mới cập nhật

		/**
		 * Sau này nếu cần show chính xác
		 */
		$dProduct['cashback_is_cash_value'] = 0;
		$dProduct['cashback_cash'] = 0;
		$dProduct['cashback_cash_by'] = 0;
		$dProduct['cashback_is_value'] = 0;
		$dProduct['cashback_value'] = 0;
		$dProduct['cashback_value_by'] = 0;
		/**
		 * Sau này nếu cần show chính xác
		 */

		$dProduct['point_is_value'] 	= 0;
		$dProduct['point_value'] 		= 0;
		if( isset($dCashBack['member_group_id']) ){

			$dWCVP 						= $wallet_cashback->cal_commission( $dCashBack, 1, $dProduct['price'], 0 );
			$dProduct['cashback_vnd'] 	= $dWCVP['vnd'];
			$dProduct['cashback_point']	= $dWCVP['point'];
			
			/**
			 * Sau này nếu cần show chính xác
			 */
			$dProduct['cashback_is_cash_value'] 		= $dCashBack['is_cash_value'];
			$dProduct['cashback_cash'] 					= $dCashBack['cash'];
			$dProduct['cashback_cash_by'] 				= $dCashBack['cash_by'];

			$dProduct['cashback_is_value'] 				= $dCashBack['is_value'];
			$dProduct['cashback_value'] 				= $dCashBack['value'];
			$dProduct['cashback_value_by'] 				= $dCashBack['value_by'];
			/**
			 * Sau này nếu cần show chính xác
			 */

			$dProduct['point_is_value'] 		= $dCashBack['is_point_value']+0;
			$dProduct['point_value'] 			= $dCashBack['point']+0;
			//điểm là % giá tiền để show giá đổi điểm
			if( $dProduct['point_is_value'] == 1 ){
				$dProduct['point'] 					= $dProduct['point_value'];
			}else{
				$dProduct['point'] 					= ($dProduct['price']*$dProduct['point_value'])/100;
			}
			unset($dCashBack);
		...
		//Show danh sách chiết khấu ở những nhóm khác
		$dProduct['list_decrement'] = array();//danh sách chiết khấu
		if( isset($setup['member_title_show_all_decrement']) ){
			$lDec = explode(';', $setup['member_title_show_all_decrement']);//member_title_show_all_decrement = danh sách được show
			if( isset($dClient['member_title_id']) && in_array( $dClient['member_title_id'] , $lDec ) ){
				$product_commission_detail->set('product_commission_id', $dProduct['product_commission_id']);
				$product_commission_detail->set('member_group_id', $setup['member_group_showed_detail_product']);//member_group_showed_detail_product => show trên chi tiết sản phẩm
				$dProduct['list_decrement'] = $product_commission_detail->list_by_member_group();
			}
        }

        //Show danh sách cashback ở những nhóm khác
		$dProduct['list_cashback'] = array();//danh sách cashback các cấp
		if( isset($setup['member_title_show_all_cashback']) ){
			$lDec = explode(';', $setup['member_title_show_all_cashback']);//member_group_showed_detail_product_cashback = danh sách được show
			if( isset($dClient['member_title_id']) && in_array( $dClient['member_title_id'] , $lDec ) ){
				$wallet_cashback->set('product_commission_id', $dProduct['product_commission_id']);
				$wallet_cashback->set('member_group_id', $setup['member_group_showed_detail_product_cashback']);//member_group_showed_detail_product_cashback => show trên chi tiết sản phẩm
				$dProduct['list_cashback'] = $wallet_cashback->list_by_member_group();
			}
        }

		//Chi tiết bảng giá show theo giá từng đối tượng
		$dProduct['list_price'] = array();//danh sách giá các cấp
		if( isset($setup['member_title_show_list_price']) ){
			$lDec = explode(';', $setup['member_title_show_list_price']);//member_title_show_list_price = danh sách được show danh sách giá
			if( isset($setup['enabled_apply_discount_on_price']) && $setup['enabled_apply_discount_on_price'] == 1 ){
				$product_price_detail->set('product_id', $dProduct['product_id']);
				$product_price_detail->set('unique_id', $dProduct['unique_id']);
				$product_price_detail->set('member_group_id', $setup['member_group_showed_list_price']);//member_group_showed_list_price danh sách giá từng đối tượng
				$dProduct['list_price'] = $product_price_detail->list_by_product_member_group_defined();
			}
		}
		
		//Nhận chiết khấu trực tiếp bao gồm 2 loại điểm và vnd
		$dComPro = $this->cal_commission_by_product( $member_group_id, $dProduct['product_commission_id'], $dProduct['price_cal_commission'], 1, 0 );
		$dProduct['commission_coaching'] 	= $dComPro['coach_vnd'];//SAUSEXOA
		$dProduct['commission_vnd'] 		= $dComPro['coach_vnd'];
		$dProduct['commission_point'] 		= $dComPro['coach_point'];
		$dProduct['referral_coaching'] 		= $dComPro['referral_vnd'];//SAUSEXOA
		$dProduct['referral_vnd'] 			= $dComPro['referral_vnd'];
		$dProduct['referral_point'] 		= $dComPro['referral_point'];
		
		$dProduct['showroom_coaching']		= 0;//SAUSEXOA
		$dProduct['showroom_vnd']			= 0;
		$dProduct['showroom_point']			= 0;
		if( isset($dClient['user_id']) ){
			$showroom 					= new showroom();
			$commission_group_detail 	= new commission_group_detail();

			$showroom->set('owner', $dClient['user_id']);
			$dSR = $showroom->get_by_client();
			if( isset($dSR['id']) && $dSR['commission_group_id'] > 0 ){
				//BEGIN Tính hoa hồng giới thiệu => bằng VNĐ và point
				$commission_group_detail->set('commission_group_id', $dSR['commission_group_id']);
				$commission_group_detail->set('product_commission_id', $dProduct['product_commission_id'] );
				$d = $commission_group_detail->cal_commission( $dProduct['price_cal_commission'], 1, 0 );

				$dProduct['showroom_coaching']		= $d['coach_vnd'];//SAUSEXOA
				$dProduct['showroom_vnd']			= $d['coach_vnd'];
				$dProduct['showroom_point']			= $d['coach_point'];
			}

		}

		//BEGIN tạo link referral
		if( isset($setup['domain_web']) && $setup['domain_web'] != '' ){
			$dProduct['link_referral']		= $setup['domain_web'].'/'.$main->url($dProduct['product_name']).'-p'.$dProduct['unique_id'];
		}else{
			$dProduct['link_referral']		= $link.'/'.$main->url($dProduct['product_name']).'-p'.$dProduct['unique_id'];
		}
		if( isset($dClient['user_id']) ){
			$dProduct['link_referral']		= $dProduct['link_referral'].'/'.$dClient['user_id'];
		}
		// print_r($dProduct);
		// exit();
		return $dProduct;
	...
	public function cal_commission_by_product( $member_group_id, //KPI.
		...
		if( isset($dMemberGroup['id']) ){
			...
			/**
			 * BEGIN hoa hồng giới thiệu trực tiếp
			 */
			$commission_group_id = $dMemberGroup['commission_group_id'];

			if( $product_commission_id > 0 && $commission_group_id > 0 ){

				//BEGIN Tính hoa hồng giới thiệu => bằng VNĐ và point
				$commission_group_detail->set('commission_group_id', $commission_group_id);
				$commission_group_detail->set('product_commission_id', $product_commission_id );
				$d = $commission_group_detail->cal_commission( $price, $quantity, $decrement );
				
				$coach_vnd 			= $d['coach_vnd'];
				$coach_point 		= $d['coach_point'];

			}
			/**
			 * END hoa hồng giới thiệu trực tiếp
			 */

			 /**
			 * BEGIN hoa hồng giới thiệu gián tiếp
			 */
			$commission_group_id = $dMemberGroup['commission_group_id_ref'];
			...
			if( $product_commission_id > 0 && $commission_group_id > 0 ){
				...
				$referral_vnd 		= $d['coach_vnd'];
				$referral_point 	= $d['coach_point'];

			}
			/**
			 * END hoa hồng giới thiệu gián tiếp
			 */

		}

		return array( 
					'coach_vnd'=> $coach_vnd, 'coach_point'=> $coach_point, 
					'referral_vnd'=> $referral_vnd, 'referral_point'=> $referral_point 
				);
	...
	public function list_all_product_on_sales($keyword, $shop_id, $member_group_id, $offset, $limit)
	{
		global $db, $main, $product_commission_detail, $product_commission;
	...
	// Danh sách sản phẩm theo category limit 0,8 phần danh mục
	public function list_related( $member_group_id = '', $limit = 10 )
	{
		global $db;
