...
class theme_category extends model
{
    ...
    public function get_detail_item()
    {
        ...
        if (isset($d['id'])) {
            ...
            foreach ($d['img_list'] as $key => $item) {
                if ($item['type'] == 'category') {
                    if (isset($item['type_value'])) {
                        $this->set('id', $item['type_value']);
                        $dSlide = $this->get_detail();
                        $url    = '';
                        $name   = '';
                        if (isset($dSlide['id'])) {
                            $url    = $main->convert_link_url($dSlide['name']);
                            $url    = $url . '-c' . $dSlide['id'];
                            $name   = $dSlide['name'];
                        }
                        $item['name']     = $name;
    ...
    public function update_child_root_branch()
    { //Cập nhật toàn bộ nhánh qua nhánh mới; chính bản thân nó đã được update rồi $id
        global $db;
    ...
    public function filter_category_count()
    { //with sub list item
        global $db;
    ...
    public function list_cat_by_parent($offset = '', $limit = '')
    {
        ...
        while ($row = mysqli_fetch_assoc($r)) {
            $row['cat_link'] = $main->convert_link_url($row['name']);
            $kq[] = $row;
        }
