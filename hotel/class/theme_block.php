...
class theme_block extends model
{
    ...
    public function get_detail_item($member_group_id = 0)
    {
        ...
        if ($d['slide'] > 0) {
            ...
            foreach ($d['slide_list'] as $key => $item) {
                if ($item['type'] == 'category') {
                    $url    = '';
                    $name   = '';

                    if (isset($item['type_value']) && $item['type_value'] != '') {
                        $theme_category->set('id', $item['type_value']);
                        $dSlide = $theme_category->get_detail();
                        if (isset($dSlide['id'])) {
        ...
        $d['cat_link'] = $main->convert_link_url($d['name']);
