    ...
    if (itemCart != null && itemCart.length > 0) {
        ...
        _doAjaxNod('POST', '', 'cart_payment', 'info', 'infoShip', true, (res) => {
            ...
            if (!res.data.l) {
                ...
                thePage.contentAddress.append(thePage.renderAddress(res.data.l));
...
thePage.renderAddress = (l) => {
    if (l.length > 0) {
        ...
        h += `<div class="text-right">
                <div class="btn btn-key btn-width" type="submit" id="btn_shipments" name="btn_shipments">
...
thePage.feeShip = () => {
    _doAjaxNod('POST', '', 'cart_payment', 'fee', 'ship', true, (res) => {
        ...
        thePage.totalPayment.html(thePage.renderTotalPayment(thePage.sumTotal()));
    });
}
...
thePage.renderTotalPayment = (total) => {
    h = `<li><span>Tổng tiền hàng:</span> <p>${number_format_replace_cog(total)} đ</p></li>
        <li><span>Phí vận chuyển:</span> <p>${thePage.fee_ship > 0 ? number_format_replace_cog(thePage.fee_ship) : 'Liên hệ'}</p></li>
        <input type="text" hidden id="ship_fee" value="${thePage.fee_ship}"/>
        <li><span style="position: relative;top: 8px;">Tổng thanh toán:</span> <p>${number_format_replace_cog(total+parseFloat(thePage.fee_ship))} đ</p></li>`;
...
$('body').on('click', '#btn_shipments, #btn_shipments_form', function(e) {
    loadShipment();
    document.getElementById("loading_bar").style.display = "block";
    // $('#modalShipments').modal('show');
});

function loadShipment() { // onchange district theo city tạo showroom mới
    ...
    _doAjaxNod('POST', data, 'cart_payment', 'shipments', 'list', false, (res) => {
        $('#list_shipments').html(thePage.renderShipments(res.data));
        $('#modalShipments').modal('show');
        document.getElementById("loading_bar").style.display = "none";
    });
}

thePage.renderShipments = (l) => {
    let h = '';
    l.forEach(function(item) {
        h += `<tr>
                <td id="optionShipments" id_shipments="${item.id}" price_shipments="${item.total_fee}"s><img src="${item.carrier_logo}" width="80"/></td>
                <td id="optionShipments" id_shipments="${item.id}" price_shipments="${item.total_fee}"><a>${item.carrier_name}</a></td>
                <td id="optionShipments" id_shipments="${item.id}" price_shipments="${item.total_fee}"><a>${item.service}</a></td>
                <td id="optionShipments" id_shipments="${item.id}" price_shipments="${item.total_fee}"><a>${number_format_replace_cog(item.total_fee)}</a></td>
                <td id="optionShipments" id_shipments="${item.id}" price_shipments="${item.total_fee}"><a>${item.expected}</a></td>
            </tr>`;
    })
...
$('body').on('click', '#optionShipments', function(e) {
    let _id_shipments = $(this).attr('id_shipments');
    let _price_shipments = $(this).attr('price_shipments');
    $("#inp_id_shipments").attr('value', _id_shipments);
    thePage.fee_ship = _price_shipments;
    thePage.feeShip();
    $('#modalShipments').modal('hide');
});