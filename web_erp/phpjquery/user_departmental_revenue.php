	...
	if ($nod == 'reve_department') {
		...
		if (isset($dClient['user_id']) && $dClient['user_id'] != '') {
			...
			if ($from < $setup['begin_time']) {
				...
				$commission_department->set('client_mngt_revenue_id', $dClient['user_id']);//người hưởng ở bảng member_department.client_mngt_revenue_id
	
				$kq['info']         = $commission_department->client_mngt_revenue_info_sum($keyword, $from - 1, $to);
				$paging->total 		= $kq['info']['total_record'];
				$page_html 			= $paging->display('paging_this');
				$kq['l']            = $commission_department->client_mngt_revenue($keyword, '', $from - 1, $to, $field, $sort, $offset, $limit);
